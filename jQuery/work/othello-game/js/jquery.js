...
(function($) {
    $.fn.othelloGame = function() {
        ...
        function initializeBoard() {
            for (let row = 0; row < boardSize; row++) {
                ...
                for (let col = 0; col < boardSize; col++) {
                    const cell = $("<td></td>").attr("data-row", row).attr("data-col", col);
        ...
        function setPiece(row, col, color) {
            board[row][col] = color;
            const piece = $("<div></div>").addClass("piece").addClass(color);
            $(`td[data-row=${row}][data-col=${col}]`).html(piece);
        }
        ...
        tableBody.on("click", "td", function() {
            ...
            if (board[row][col] === "" && isValidMove(row, col, "white")) {
                setPiece(row, col, "white");
                flipPieces(row, col, "white");
                setTimeout(cpuTurn, 500); // CPUのターンへ
            }
        });
        ...
        function cpuTurn() {
            for (let row = 0; row < boardSize; row++) {
                for (let col = 0; col < boardSize; col++) {
                    if (board[row][col] === "" && isValidMove(row, col, "black")) {
                        setPiece(row, col, "black");
        ...
        function flipPieces(row, col, color) {
            const flippablePieces = getFlippablePieces(row, col, color);
            flippablePieces.forEach(([r, c]) => setPiece(r, c, color));
        }
